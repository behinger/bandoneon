...
createChords = function(octave,type){
  var val = keys.map(function(inNote){
    ...
    return Tonal.chord(inNote+octave+" "+type).map(function(x){return(Tonal.note.simplify(x).toLowerCase())})
  })
...
chords["left"]["open"] = {}
chords["left"]["open"]['M']  = createChords("1","M");
chords["left"]["open"]['m']  = createChords("1","m");
